...
async function draftProspectEmail({ businessName, niche, findingMessage, findingUrl, proofLine, variant }) {
  const variantBrief =
    variant === 'B'
  ...
  const userContent = [
    ...
    `Affected page: ${findingUrl || 'not specified — describe the defect without naming a specific page'}`,
