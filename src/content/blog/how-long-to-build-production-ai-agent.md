...
<p>At TelaLoom, we treat the LLM call as the easiest, least important part of the system. The actual work is building the scaffolding around it. When we built <a href="/projects/emailtriage.html">Email Triage</a>, the core prompt was the quick part; most of the work was the sync workers, job queue, retries and fallbacks around it.</p>

<p>First, you have to invert your architecture. Stop trying to build an AI that uses tools, and start building a traditional, deterministic software application that occasionally calls an LLM to make a routing decision or transform unstructured text. When a new payload arrives in Email Triage, we don't just dump the raw text into a giant conversational context window. We extract the metadata deterministically. We use an LLM specifically to classify the email and score its priority. If the model says an email needs a reply but returns no draft, code — not the prompt — decides to ask again. The LLM is a function call, not the brain of the entire application. The agent's state lives in your database, not in the LLM's context window.</p>
