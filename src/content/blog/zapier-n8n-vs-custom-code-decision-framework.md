...
<p><strong>1. Is the workflow reasoning, or just routing?</strong> If every step is deterministic — this field maps to that field, this trigger fires that action — a visual workflow tool is the right call, full stop. The moment a step needs to make a judgment call on ambiguous input (is this email actually a complaint, does this resume match the role, should this draft get sent as-is), you need code, because you need to control the prompt, log the model's reasoning, and write <a href="/blog/why-we-run-evals-before-shipping-ai-features.html">real evals</a> against it. On our <a href="/projects/emailtriage.html">Email Triage</a> project, the initial version was an n8n flow with an LLM node classifying incoming support tickets. It worked until we needed confidence thresholds, a fallback to human review, and a way to A/B test two different classification prompts against real traffic. None of that was possible without breaking out of the visual canvas.</p>
