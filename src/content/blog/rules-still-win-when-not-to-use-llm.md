...
description: "LLMs aren't a silver bullet. We break down TelaLoom's decision tree for when deterministic rules outperform AI models."
...
<h2>TelaLoom's Decision Tree: Rules vs. LLMs</h2>
<p>At TelaLoom, before we write a line of code for an AI-powered feature, we run through a decision tree. This helps us choose the right tool for the job. We use this framework for client projects and our own products alike, whether it’s for <a href="https://telaloom.com/projects/brightpath.html">BrightPath</a>’s tutor or <a href="https://telaloom.com/projects/cv-matcher.html">CV Matcher</a>’s scoring.</p>
<ol>
<li><strong>Is the Output Deterministic?</strong>
<ul>
<li><strong>If YES: Use Rules.</strong> If the same input should *always* produce the exact same output, an LLM is overkill and unreliable. Examples: Validating an email address (regex), parsing a known JSON schema (Pydantic), calculating a discount based on fixed criteria (if/else logic), extracting a specific ID from a fixed-format string. <a href="https://telaloom.com/projects/autoarchive.html">AutoArchive Mail</a> works this way end to end: it keys on message IDs and fixed-pattern case IDs, with no LLM at all.</li>
...
<li><strong>If LOOSE (hundreds of ms or seconds): Consider LLM.</strong> If the user can wait, or the task runs asynchronously (e.g., scheduled background processing), LLM latency is acceptable.</li>
</ul>
</li>
...
<p>We often find the best solution is a hybrid. Rules serve as guardrails, pre-processors, and post-processors, while LLMs handle the truly fuzzy, complex parts. For example, <a href="https://telaloom.com/projects/cv-matcher.html">CV Matcher</a> uses Claude for the nuanced skill match and a deterministic TF-IDF and keyword-overlap score as the fallback when the model call fails.</p>
