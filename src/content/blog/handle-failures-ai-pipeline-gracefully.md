...
<p>The try/catch catches the exception, sure. But "catch and log" isn't a strategy — it's a shrug. The pipeline stalls, the user sees a spinner or a generic error, and someone finds out three days later when a customer complains. A classifier that works fine until a vendor starts sending auto-generated receipts with malformed HTML can back up an entire triage queue overnight without anyone noticing.</p>
