...
<!-- TODO(andrew): still open — the one number this page doesn't have is
          ...
          <script>
            ...
            // Falls back to the value already in the markup above if the fetch fails.
            fetch('/api/project-stats')
              .then((r) => (r.ok ? r.json() : null))
              .then((data) => {
                const s = data && data.shuttersmith;
                if (!s) return;
                const el = document.getElementById('shuttersmith-live-visits');
                if (el) el.textContent = `${s.visits.toLocaleString()} real visits (as of ${s.asOf})`;
              })
              .catch(() => {});
          </script>
